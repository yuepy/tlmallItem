(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control21_5RzVCo: function (elem) {
      if (!elem) {
        return;
      }var tIndex = [];var data = [];var labels = elem.querySelectorAll('label');for (var k = 0; k < labels.length; k++) {
        var name = labels[k].textContent;tIndex.push({ name: name, index: k });
      }var vals = elem.querySelectorAll('.textbox-text');for (var j = 0; j < vals.length; j++) {
        var val = vals[j].value;data.push({ name: tIndex[j].name, val: val });
      }return data;
    },
    ...
    getData_control30_W5OlNt: function (elem) {
      if (!elem) {
        return;
    ...
    doAction_uiControl28_3KydCh: function (data, elem) {
    ...
    getTemplate_uiControl28_3KydCh: function () {
