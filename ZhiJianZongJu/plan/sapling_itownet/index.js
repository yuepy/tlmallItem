(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control30_2NTUE1: function (elem) {
      ...
      }obj.etcA = [];obj.etcB = [];var etcA = redio[0].querySelectorAll('div');
      var etcB = redio[1].querySelectorAll('div');for (var i = 0; i < etcA.length; i++) {
    ...
    doAction_uiControl28_ACEoFq: function (data, elem) {
      var type = data.eventType;var data = data.customData; // var elem = elem.querySelector('iframe').contentWindow.document;
      var elem = elem.ownerDocument;var elema = elem.querySelector('#enterprise-info');var redio = elem.querySelectorAll('.combo-panel');var trs = elema.querySelector('#queryDiv').querySelectorAll('tr');if (type == 'actA') {
        ...
        if (data.type == '抽查任务编号') {
          ...
          var inputs = trs[1].children[3].querySelector('.textbox-text');inputs.focus();$(inputs).val(data.target);
    ...
    getData_control35_z11Gzi: function (elem) {
      if (!elem) {
        return;
      }return elem.querySelector(".window-body") && elem.querySelector(".window-body").children[1].textContent;
    },
