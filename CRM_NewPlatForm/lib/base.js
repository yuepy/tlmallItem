...
(function (win, ysp) {
 ...
              var MenuList = JSON.parse(LoginXhr.response).listMenu;
              ...
              if (currentAwin.frameElement && currentAwin.frameElement.name == "browserFrame2" && currentAwin.frameElement.dataset.browser) {
                if (currentAwin.location.href.indexOf('login') !== -1) {
                  //currentAwin.frameElement.src = 'https://vcrm-uat.pttl.com:8080/pttlCrm/res/index.html'
                  //ysp.runtime.Model.setForceMatchModels(['index']);
                }
              }
              currentAwin.location.href = 'https://vcrm-uat.pttl.com:8080/pttlCrm/res/index.html';
              getAllMenu(currentAwin,MenuList);
              ysp.appMain.hideLoading();
            }
          }
          LoginXhr.open('POST','https://vcrm-uat.pttl.com:8080/pttlCrm/login/loginInForMobile');
          LoginXhr.send(body);
          ysp.appMain.showLoading();
        }
      }
      EnCoderXhr.open('POST','https://vcrm-uat.pttl.com:8080/pttlCrm/login/getEncoderForMobile');
      EnCoderXhr.send();
    }
  }
 ...
      if(xhr.status >= 200 && xhr.status <300 || xhr.status == 304){
    ...
    xhr.open('POST','https://vcrm-uat.pttl.com:8080/pttlCrm/crm/workSummary/getWorkBenchSummaryCount');
    xhr.send()
  }
  // 请求首页面所有一级\二级\三级菜\\\\\                                                                                                              单
 ...
    if(aWin.localStorage.listMenuForMobile && !loginFlag && !menuList){
      ...
      if(AllMenu == '' || AllMenu == '{"isHaveSession":"no"}'){
        ...
        if(top.EAPI.isIOS()){
          ...
          //       encoderXHR.open('GET','https://vcrm-uat.pttl.com:8080/ptDataShow/login/crmLogin?filter_userId='+userId+'&encoder='+encoder,false);
          ...
          //     }
          //   }
          //    SessionXhr.open('GET','https://vcrm-uat.pttl.com:8080/pttlCrm/homepage/getUserIdAndEncoder',false);
          //    SessionXhr.send();
          // }
      }
    }
...
//     }
 ...
    if(url && !model){
      ...
      currentWin.location.href = 'https://vcrm-uat.pttl.com:8080/pttlCrm/res/page/visitManager/customerWorkspace/customerWorkspace.html';
    }
    if(url=='' && model){
 ...
      //调试IOS登录框问题 - 地址变成特殊地址 
      if(aWin.location.href.indexOf('ysp_mobile')!==-1){
        aWin.location.href = 'https://vcrm-uat.pttl.com:8080/pttlCrm/login';
        top.EAPI.postMessageToNative('IOSLoginIn', '');
 ...
      if (aWin.location.href == 'https://vcrm-uat.pttl.com:8080/pttlCrm/res/index.html' && !top.EAPI.isIOS()) {
          //在登录成功时,请求菜单接口,获取全部菜单列表
          getAllMenu(aWin);
      }
      ...
      // if(aWin.location.href.indexOf('ysp_mobile') !== -1){
      //   aWin.location.href = 'https://vcrm-uat.pttl.com:8080/pttlCrm/login?clientType=ysp'
      // }
