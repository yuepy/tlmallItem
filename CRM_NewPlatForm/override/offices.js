            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(bizUnits) {
	                        ...
	                        html += '                <tbody>';
	                       	...
	                       	for (var j = 0; j < secondLevel.length; j++) {
	                       		if(secondLevel[j].department==firstLevel[i].department){
	                       			var url = getLink("projectName",secondLevel[j].projectName,"06");
                        ...
                        html += '    <div class="u-title">';
                        html += '        <i class="icon-line"></i>';
                        html += '        <h2>销售人员销量与销售额</h2>';
                        html += '    </div>';
                        html += '    <div class="m-box">';
                        html += '        <div class="table-content branch-sales">';
                        html += '            <table class="table u-table-b" id="salerMan-table">';
                        html += '                <thead>';
                        html += '                    <tr><th>销售人员</th><th>销量（台）</th><th>销售额（万）</th></tr>';
                        html += '                </thead>';
                        html += '                <tbody id="salerManTable">';
                        html += '                </tbody>';
                        html += '            </table>';
                        html += '         </div>';
                        html += '         <a href="javascript:;" class="btn-display" onclick="displayTable2(this)"><span>展开</span><i class="icon"></i></a>';
                 ...
                 // 项目表格
                    ...
                    if(salerName) {
                        for (var i = 0; i < salerName.length; i++) {
                        	if(salerName[i].level==1)
                        		continue;
                        	var url = getLink("salerName",salerName[i].salesman_id,"07");
                            var html = '<tr><td><a href="'+url+'" title="' + salerName[i].salerName + '">' + salerName[i].salerName + '</a></td><td>' + toQfw_new(salerName[i].qty,false) + '</td><td>' + toQfw_new(salerName[i].amt.toFixed(2),false) + '</td></tr>';
                            $("#salerManTable").append(html);
                        }
                    }
                    tableSH("salerMan-table", tr_minH-2);
