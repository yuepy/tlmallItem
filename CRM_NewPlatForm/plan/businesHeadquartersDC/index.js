    ...
    getData_control349_5u80Vy: function (elem) {
      if (!elem) {
        ...
        var data = [];var trs = elem.querySelector("#table_list_1").querySelectorAll("tr");var reportId = elem.querySelector('input[name="reportId"]').value;var titleIdx = null;var deepFlag = true;var strFuc = function (_obj) {
          ...
          var _tds = trs[1].querySelectorAll("td");var a, b, c, d, e, f, g, h, l;if ("report34-2" == reportId || "report34-1" == reportId) {
            //销售代表（客户与门店达成）
          ...
          } else if ("report33" == reportId || "report33-1" == reportId || "report33-1-1" == reportId || "report48" == reportId) {
            //办事处主任（办事处事业部达成）
            if ("report33" == reportId) {
              for (var n = 0; n < _tds.length; n++) {
          ...
          var tds = trs[i].querySelectorAll("td");var obj = {};if (!trs[i].querySelectorAll('a')) {
            ...
            var _str = strFuc(tds[j]);var _textContent = tds[j].textContent.trim();if ("report34-2" == reportId || "report34-1" == reportId || "report18-1" == reportId || "report31-1-1-1" == reportId) {
              var titles = tds[titleIdx].textContent.trim();if ("" == titles) {
                titles = tds[titleIdx + 1].textContent.trim();
              }obj.title = titles;
            } else if ("report49" == reportId || "report33-1-1" == reportId || "report48" == reportId || "report16-1" == reportId || "report46" == reportId || "report29-1" == reportId || "report45" == reportId) {
              var titles = tds[titleIdx].textContent.trim();if ("" == titles) {
              ...
              obj.title = tds[titleIdx].textContent.trim();
                ...
                }obj.rank.num = parseInt(_textContent);break;}
          }data.push(obj);
        }return data;
      }
    },
