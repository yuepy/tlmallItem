        ...
        var imgBoxs = boxs.querySelectorAll(".img-box");for (var i = 0; i < imgBoxs.length; i++) {
          var srcs = [];var titles = [];var content = [];var imgType = imgBoxs[i].querySelector(".img-tit").textContent;var lis = imgBoxs[i].querySelector(".img-ul").querySelectorAll("li");content.push(imgType);if (lis.length > 0) {
            for (var j = 0; j < lis.length; j++) {
              ...
              var scrTitle = lis[j].querySelector("img").getAttribute('src');var scrTitleSplit = scrTitle && scrTitle.split("upload-dir/")[1];var srcUrl = decodeURI(scrTitleSplit);var scrC = "http://192.168.1.227/pttlCrm/" + srcUrl;src.push(scrC);srcs.push(src);var images = { titles: titles, srcs: srcs, content: content };
            }data.push(images);
          }
