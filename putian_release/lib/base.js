(function(win, ysp) {
  ...
  utils.extend(ysp.customHelper, {
     ...
     */
    _urlFilter: function(url) {
    	if(window.top.EAPI.isIOS() && url.indexOf('http://192.168.0.200:8080') === -1) {
      	window.top.EAPI.openWindow(url, "adapter");
        return true;
      }
      return false;
    },
    ...
    getFormData: function(elem) {
      ...
      var data = [];
      var trs = elem.rows || [];
      var typeList = ['input', 'button', 'textarea', 'a', 'br', 'select'];
      var subTitleIndexs = [];
      var titleIndexs = [];

      function parseDate(str) {
        var arr = [];
        arr.push(str.substr(0, 4), str.substr(5, 2), str.substr(8, 2));
        return arr.join('-');
      }
            ...
            if (el.style.display !== 'none' && !(el.tagName === 'INPUT' && (el.getAttribute('readOnly')))) {
              ...
              if (id) {
                ...
                if (type === 'select') {
                  obj.value = el.value;
                  obj.options = [];
                  var mySelectOpts = el.options;
                  var mySelectOptsLen = mySelectOpts.length;
                  for (var i = 0; i < mySelectOptsLen; i++) {
                    obj.options.push({
                      value: el.options[i].value,
                      text: el.options[i].innerText
                    });
                  }
                }
              }
    ...
    onTargetLoad: function(aWin, doc) {
      var confirmCode = aWin.confirm;
      aWin.confirm = function (msg) {
        var confirmFun = eval(confirmCode);
        if (!confirmFun(msg)) {aWin.top.EAPI.hideLoading(); return false;} else { return true };
      }
      if (window.top.document.querySelector('div.view-item'))
        window.top.document.querySelector('div.view-item').style.minHeight = '100vh';
      aWin.top.$ = aWin.$;
      var top = ysp.runtime.Browser.getMainBrowser().contentWindow;
      var bindFun = function (index) {
      	if (window.top.document.querySelector('.ysp-module.purchase-dialog-btn>div>button')) {
        	window.top.document.querySelector('.ysp-module.purchase-dialog-btn>div>button').onclick = function() {
            try {
              window.top.document.querySelector('div.view-item').style.overflowY = 'hidden';
              window.top.document.querySelector('div.view-item>.outer-container').style.display = 'none';
            } catch(e) {}
            
            setTimeout(function() {
              window.top.document.querySelector('.ysp-module.purchase-dialog-btn .ysp-module .icon-close.modal-icon').onclick = function() {
                try {
                  window.top.document.querySelector('div.view-item').style.overflowY = 'visible';
                  window.top.document.querySelector('div.view-item>.outer-container').style.display = 'block';  
                } catch(e) {}
              }
              window.top.document.querySelector('.distribution-header-left-btn').onclick = function() {
                try {
                  window.top.document.querySelector('div.view-item').style.overflowY = 'visible';
                  window.top.document.querySelector('div.view-item>.outer-container').style.display = 'block';
                } catch(e) {}
      ...
      bindFun(0);
      var bindFun2 = function(index) {
        if (window.top.document.querySelector('.ysp-module.hahahhha button') && !window.top.document.querySelector('.ysp-module.hahahhha button').onclick) {
          var btnlist = window.top.document.querySelectorAll('.ysp-module.hahahhha button'), btnlistLength = btnlist.length;
          for (var i = 0; i < btnlistLength; i++) {
            btnlist[i].onclick = function() {
              if (window.top.document.querySelector('[data-type="root"]') && window.top.document.querySelector('[data-type="root"]').style.overflowY === 'hidden') {
                try {
                  window.top.document.querySelector('[data-type="root"]').style.overflowY = 'visible';
                  window.top.document.querySelector('div.view-item>.outer-container').style.display = 'block';
                } catch(e) {}
              } else {
                try {
                ...
                // window.top.document.querySelector('[data-type="root"]').style.overflowY = 'hidden';
                setTimeout(function() {
                  window.top.document.querySelector('.modal-header>span.icon-close').onclick = function() {
                    try {
                      window.top.document.querySelector('[data-type="root"]').style.overflowY = 'visible';
                      window.top.document.querySelector('div.view-item').style.overflowY = 'visible';
                      window.top.document.querySelector('div.view-item>.outer-container').style.display = 'block';
      ...
      bindFun2(0);
      var bindFun3 = function(index) {
        if (window.top.document.querySelector('[data-type="stickyBody"] .ysp-module button') && !window.top.document.querySelector('[data-type="stickyBody"] .ysp-module button').onclick) {
          window.top.document.querySelector('[data-type="stickyBody"] .ysp-module button').onclick = function() {
            try {
              window.top.document.querySelector('div.view-item').style.overflowY = 'hidden';
              window.top.document.querySelector('div.view-item>.outer-container').style.display = 'none';
            } catch(e) {}
            setTimeout(function() {
              window.top.document.querySelector('.ysp-module .modal-header .icon-close.modal-icon').onclick = function() {
                try {
                  window.top.document.querySelector('div.view-item').style.overflowY = 'visible';
                  window.top.document.querySelector('div.view-item>.outer-container').style.display = 'block';
                } catch(e) {}
      ...
      bindFun3(0);
      var bindFun4 = function() {
        if (!window.top.document.querySelector('.ysp-module .modal-header .icon-close.modal-icon') || !window.top.document.querySelector('.modal-header>span.icon-close') || !window.top.document.querySelector('.ysp-module.purchase-dialog-btn .ysp-module .icon-close.modal-icon')) {
          try {
            window.top.document.querySelector('[data-type="root"]').style.overflowY = 'visible';
            window.top.document.querySelector('div.view-item').style.overflowY = 'visible';
            window.top.document.querySelector('div.view-item>.outer-container').style.display = 'block';
      ...
      if (funcName && funcName.textContent === '付款申请单') {
        aWin.top.Fix.Manager.createDialog = function(obj){
          aWin.open('http://192.168.0.200:8080/ptsoa/'+obj.url);
        };
      }
      ...
      var num = 0;
      var lxFun = function() {
      	if (num > 500 || doc.getElementById('c_category') && doc.getElementById('c_category').children[0]) {
          ...
          console.log(doc.getElementById('c_category'));
          // console.log(doc.getElementById('c_category').value);
          console.log(doc.getElementById('c_defKey'));
          // console.log(doc.getElementById('c_defKey').value);
        } else {
          setTimeout(function() {
            num++;
        		lxFun();
    ...
    isLoginSuccess: function(doc) {
      ...
      var cond2 = doc.querySelectorAll('iframe').length === 2;
      return cond1 && cond2;
    }
  });
