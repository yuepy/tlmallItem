    ...
    doAction_uiControl174_FXKnek: function (data, elem) {
      if (data.eventType == "inputclick") {
        var a = data.dataCustom;var ind = parseInt(a.ind);var clases = a.clas;var check = a.check;if (clases == "ysp-shenpiMoney one") {
          var target = elem.querySelector("#isBelowJcPrice").querySelectorAll("input")[ind];target.click();elem.querySelector("#isBelowJcPrice").querySelectorAll("input")[ind].checked = check;$($(target).parent().siblings().children("input"))[0].checked = false;
        } else {
          var target = elem.querySelector("#isBelowKsPrice").querySelectorAll("input")[ind];target.click();elem.querySelector("#isBelowKsPrice").querySelectorAll("input")[ind].checked = check;$($(target).parent().siblings().children("input"))[0].checked = false;
        }
      } else if (data.eventType == "change") {
        var obj = data.dataCustom;elem.querySelector("#ksPrice").querySelector("input").value = obj.val;elem.querySelector("#ksPrice").querySelector("input").dispatchEvent(new Event("change"));
      }
    },
