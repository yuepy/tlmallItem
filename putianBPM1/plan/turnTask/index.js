(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control95_h1V7Wk: function (elem) {
      if (!elem) {
        return;
      }const data = { processTit: [], time: { tit: [], con: [] }, base: { tit: [], con: [] }, turn: { tit: [], con: [] }, remark: {}, annex: [], surggest: [], errorTip: {} }; //各项title
      ...
      const ann = elem.querySelectorAll(".mini-panel")[3].querySelectorAll("tbody tr td a");if (elem.querySelectorAll(".mini-panel")[3].style.display == "none") {
        data.annex = "none";
      }[].map.call(ann, (item, index) => {
        data.annex.push(item.textContent);
      }); //建议
      const surgg = elem.querySelectorAll(".mini-panel")[4].querySelector("tbody tr td textarea");if (surgg) {
        data.surggest.push(surgg.placeholder);data.surggest.push(surgg.value);
      } //判断审批意见文本框是否可写
      const surggspan = elem.querySelectorAll(".table")[5].querySelector("tbody tr td .mini-textbox-readOnly");if (surggspan) {
