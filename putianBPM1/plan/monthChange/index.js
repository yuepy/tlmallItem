(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control154_UqN5eJ: function (elem) {
      if (!elem) {
        return;
      }var data = { base: { bigtit: "", tit: [], con: [] }, tablehead: [], table: [], total: { num: [], con: [] }, reason: [], annex: [], surggest: [], errorTip: {} }; //基础信息
      var baseBigtit = elem.querySelectorAll(".mini-panel")[0].querySelector(".mini-panel-title");data.base.bigtit = baseBigtit.textContent;var basetit = elem.querySelectorAll(".mini-panel")[0].querySelector(".nui-form-table").querySelectorAll("tbody tr");[].map.call(basetit, function (item, index) {
      ...
      var thead = elem.querySelector("#apply").querySelector(".mini-grid-columns-view").querySelector("tbody tr:nth-child(2)").querySelectorAll("td");[].map.call(thead, function (item, index) {
        data.tablehead.push(item.textContent);
      });data.tablehead.shift();data.tablehead.shift();var tbody = elem.querySelector("#apply").querySelector(".mini-panel-body").querySelector(".mini-grid-rows-view").querySelectorAll("tbody tr");[].map.call(tbody, function (item, index) {
        if (index != 0) {
          var td = item.querySelectorAll("td");var tr = [];[].map.call(td, function (d, i) {
            tr.push(d.textContent);
          });tr.shift();tr.shift();data.table.push(tr);
        }
      }); //总计：
      var totalTd = elem.querySelector("#apply").querySelector(".mini-grid-summaryRow").querySelector(".mini-grid-summaryRow-view").querySelector("tbody tr:nth-child(2)").querySelectorAll("td");[].map.call(totalTd, function (item, index) {
        if (item.textContent.trim() != "") {
          data.total.con.push(item.textContent.trim());data.total.num.push(index);
        }
