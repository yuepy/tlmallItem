(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control154_UqN5eJ: function (elem) {
      if (!elem) {
        return;
      }var data = { base: { bigtit: "", tit: [], con: [] }, table: [], total: [], reason: [], annex: [], surggest: [], errorTip: {} }; //基础信息
      ...
      var thead = elem.querySelector("#apply").querySelector(".mini-grid-columns-view").querySelector("tbody tr:nth-child(2)").querySelectorAll("td");var th = [];[].map.call(thead, function (item, index) {
        th.push(item.textContent);
      });th.shift();data.table.push(th);var tbody = elem.querySelector("#apply").querySelector(".mini-panel-body").querySelector(".mini-grid-rows-view").querySelectorAll("tbody tr");[].map.call(tbody, function (item, index) {
        if (index != 0) {
