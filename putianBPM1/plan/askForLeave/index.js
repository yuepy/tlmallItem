  ...
  ysp.runtime.Model.extendLoadingModel({
      ...
      if (data.eventType == 'click') {
        var d = data.dataCustom;var btn = elem.ownerDocument.querySelector('.mini-tools-close');if (d == '领取') {
          ...
          var newRow = elem.contentDocument.querySelectorAll("iframe")[0].contentDocument.querySelectorAll(".mini-grid-rowstable")[1];var reg = /\s/;if (_click && reg.test(newRow.textContent)) {
            ...
            if (_click) {
              var _icon = _click.querySelector('a');var _innerHTML = _click.querySelector('span').textContent;_innerHTML == '取消领取' && _icon.click();var json = {
    ...
    getData_control123_vwIGHi: function (elem) {
      if (!elem) {
        ...
        var data = { topTitle: "", totalHoliday: "", holidayDetail: [], resonLeave: "", enclosure: [], url: [], table: { columnTitle: [], content: [] } };if (elem.querySelector(".mini-panel-header")) {
        ...
        if (elem.querySelector("#reasonsLeave")) {
          ...
          var tempArr = enclosure.querySelectorAll("a");for (var i = 0; i < tempArr.length; i++) {
            data.enclosure.push(tempArr[i].textContent.trim());data.url.push(tempArr[i].href);
          }
        } //表格信息
    ...
    getData_control155_SF9UmQ: function (elem) {
      if (!elem) {
        ...
        var data = { topTitle: "", totalHoliday: "", holidayDetail: [], resonLeave: "", enclosure: [], table: { columnTitle: [], content: [] } };if (elem.querySelector(".mini-panel-header")) {
          data.topTitle = elem.querySelector(".mini-panel-header").textContent.trim();
        }if (elem.querySelector("#holidayDays")) {
          data.totalHoliday = elem.querySelector("#holidayDays").textContent.trim().match(/\d.*/g); //请假详情
        }var holiday = elem.querySelector("#holidayDetail");if (holiday) {
