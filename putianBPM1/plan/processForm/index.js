    ...
    ysp.runtime.Model.extendLoadingModel({
        ...
        getData_control21_GeryA0: function(elem) {
            ...
            if (elem && elem.contentWindow && elem.contentWindow.document) {
                var data = { onePartTitle: [], onePartCont: [], twoPart: {}, twoPartTitle: [], twoPartCont: [], threePartTitle: [], threePartCont: [], twoPartAdd: [], Agree: [], total: [], totalTitle: [], totalTF: {}, btns: { btn: [], btnID: [] }, file: [], rejectsStatus: { Retit: [], Recont: [] }, belowKsPeice: { Retit: [], Recont: [] }, tishi: { title: [], cont: [] }, receiveType: [], XTHJG: { DetailTitle: [], title: [], cont: [], total: [] }, THJL: { DetailTitle: [], title: [], cont: [], total: [] }, KCBJ: { DetailTitle: [], title: [], cont: [], total: [] }, ZDJL: { DetailTitle: [], title: [], cont: [], total: [] }, QQTHBC: { DetailTitle: [], title: [], cont: [], total: [] } }; /****B2B销售订单流程里receiveType选择收款方式****/ //PC 为伪装  select ，PC点击id为receiveType下.mini-buttonedit-icon id为mini-38x下table td为所选内容，点击receiveType内容更新。
                ...
                var btbAddr = elem.contentWindow.document.querySelector('#btbAddr'); //data.btns 保存按钮标记 B2B销售订单A链接按钮  PC存在四个id的情况 故 取四种情况。
                // 保存按钮文本内容及id,对应模板 EssentialInformation 中 btns。
                /*************付款申请单的增加删除按钮3.8***************/
                    ...
                    if (toolbar && toolbar.style.display != 'none') {
                        ...
                        if (_td.length > 1) {
                            if (!_td[0].querySelector('a') && _td[0].textContent.trim() != '') {
                                switch (_id.id) {
                                    default: data.totalTitle.push(_td[0].textContent.trim());
                                    break;
                                }
                            } else if (_td[0].querySelector('a') && _td[1].textContent.trim() != '') {
                                switch (_id.id) {
                                    default: data.totalTitle.push(_td[1].textContent.trim());
                                    break;
                                }
                            } else {
                                switch (_id.id) {
                                    default: data.totalTitle.push(title.textContent.trim());
                                    break;
                                }
                            }
        ...
        doAction_uiControl22_QmOw34: function(data, elem) {
            if (data.eventType == 'change') {
                ...
                switch (d[0]) {
                    case 'btn btn-xs btn-primary btn-hollow detail':
                        ...
                        if (d[2] != null) {
                            ...
                            var _btn = elem.contentWindow.document.querySelectorAll('' + '#' + d[6] + '	.mini-button' + '')[d[1]]; /*******************************************合同里信用及信用按钮*******************************************************************/
                        }
                        _btn.click();
                ...
                if (d == "svpCfo") {
                    ...
                    window._input = _input;
                    window._input.focus();
                    window._input.dispatchEvent(new MouseEvent('mousedown', {
                        bubbles: true,
                        cancelable: true,
                        view: window._input.ownerDocument.defaultView
                    }));
                    window._input.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, cancelable: true, view: window._input.ownerDocument.defaultView }));
                    window._input.click();
