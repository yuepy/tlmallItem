(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control193_N9CBTD: function (elem) {
      if (!elem) {
        return;
      }var content = ysp.customHelper.getTableData(elem, ['流程名称', '流程编号', '内容摘要', '申请人', '停留时间', '申请日期']);var arr = ['要事追踪申请单'];var data = ysp.customHelper.getDatamove(content, arr);content.content = data.data;content.index = data.index;return content;
    },
    doAction_uiControl193_bw3nWM: function (data, elem) {
      var type = data.eventType,
          data = data.customData;if (type == 'td_click') {
        var arr = ['请假申请单', '销假申请单', '离职申请单', '加班申请单', '因公外出申请单', '考勤异常申请单', '员工异动申请单', '员工异动申请单V2', '招聘申请单', '员工合同续签申请单', '试用期转正申请单', '录用入职申请单', '培训申请单', '人才盘点信息表', '资产预算调整流程', '资产预算调整流程V2', '信用冻结订单释放申请流程', '资产类预算外申请流程V2', '借款/预付款申请单', '资产处理申请单', '差旅费用报销流程', '费用报销流程', '路线新增流程', '路线变更流程', '路线组变更申请', '客资/送大方/销售人员.路线批量审批', '市场用品采购流程', '总部合同审批流程', '用印申请流程', '合同审批流程', '客资批量审批', '送达方批量审批', '路线批量审批', '通用流程（知会）', '通用流程']; // debugger;
        var flat = arr.indexOf(data.text);var hreflo = elem.querySelectorAll('tr')[++data.id].querySelector('a').onclick.toString().split("ShowPage('")[1].split("')")[0]; // debugger;
        if (flat == -1) {
          if (hreflo[0] == 'h') {
            hreflo += "&_ysp_forcepc=1";ysp.appMain.openWindow(hreflo); // location.href = hreflo;
          } else {
          ...
          // debugger; 
          // location.href = hreflo;
          // var hreflo;
          // hreflo = elem.querySelectorAll('tr')[++data.id].querySelector('a').onclick.toString().split("ShowPage('")[1].split("')")[0];
          elem.ownerDocument.defaultView.open(hreflo); // elem.querySelectorAll('tr')[++data.id].querySelector('a').click();
        }
      }return false;
    },
    ...
    getData_control240_SJr66K: function (elem) {
      if (!elem) {
        return;
    ...
    doAction_uiControl240_ur23qy: function (data, elem) {
      if (data.eventType == 'onchange') {
        ...
        var val = data.dataCustom[1];var target = elem.querySelector("input[name='" + selId + "']");target.value = val;target.focus();
    ...
    getTemplate_uiControl240_ur23qy: function () {
    ...
    getData_control2_2DedGV: function (elem) {
      //console.log(elem);
      if (!elem) {
        return "";
      }if (elem) {
    ...
    doAction_uiControl2_iAH5Jo: function (data, elem) {
