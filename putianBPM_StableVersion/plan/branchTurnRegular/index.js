(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    doAction_uiControl197_W0sNDl: function (data, elem) {
      if (data.eventType == 'click') {
        var d = data.dataCustom;var btn = elem.ownerDocument.querySelectorAll('.mini-tools-close')[1];if (d == '领取') {
          ...
          //     } else {
          if (_click) {
            var _icon = _click.querySelector('a');var _innerHTML = _click.querySelector('span').textContent;
            _innerHTML == '取消领取' && _icon.click();var json = { time: new Date().getTime() };ysp.appMain.getActiveWindow().history.pushState(json, "", "/ptsoa/bps/wfclient/task/app/taskTabPage/pendingTask.jsp?");
          } else {
    ...
    doAction_uiControl206_Qs6imk: function (data, elem) {
      if (data.eventType == "blur") {
        elem.querySelector("textarea").value = data.dataCustom;elem.querySelector("textarea").dispatchEvent(new Event("change"));
      }
    },
    ...
    getData_control63_FwsL8C: function (elem) {
      //loading 加载。
      if (!elem) {
        return;
      }if (elem) {
    ...
    doAction_uiControl51_VCTu8i: function (data, elem) {
    ...
    getTemplate_uiControl51_VCTu8i: function () {
