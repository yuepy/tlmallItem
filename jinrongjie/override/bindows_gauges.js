...
if (!Array.prototype.filter) {
    Array.prototype.filter = function(f, obj) {
        var l = this.length;
        var res = [];
        for (var i = 0; i < l; i++) { if (f.call(obj, this[i], i, this)) { res.push(this[i]); } }
        return res;
    };
}
...
if (!Array.prototype.some) {
    Array.prototype.some = function(f, obj) {
        var l = this.length;
        for (var i = 0; i < l; i++) { if (f.call(obj, this[i], i, this)) { return true; } }
        return false;
    };
}
if (!Array.prototype.every) {
    Array.prototype.every = function(f, obj) {
        var l = this.length;
        for (var i = 0; i < l; i++) { if (!f.call(obj, this[i], i, this)) { return false; } }
        return true;
    };
}
String._rExpTrim = /(^\s+)|\s+$/g;
String.prototype.trim = function() { return this.replace(String._rExpTrim, ""); };
...
String.prototype.endsWith = function(s) { return this.substring(this.length - s.length, this.length) == s; };
String.EMPTY = "";
...
Function.prototype.addProperty = function(sName, nReadWrite) {
    ...
    if (nReadWrite & Function.READ) {
        p["get" + capitalized] = function() { return this[sName]; };
    }
    if (nReadWrite & Function.WRITE) {
        p["set" + capitalized] = function(v) { this[sName] = v; };
    }
};
...
BiObject.toHashCode = function(o) {
    if (o.hasOwnProperty("_hashCode"))
        return o._hashCode;
    return o._hashCode = "_" + (BiObject._hashCodeCounter++).toString(32);
};
BiObject.prototype.getDisposed = function() { return this._disposed; };
BiObject.prototype.getId = function() { return this._id; };
BiObject.prototype.setId = function(v) { this._id = v; };
...
_p.disposeFields = function(fieldNames) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (this.hasOwnProperty(n)) {
            o = this[n];
            if (o != null) { if (typeof o.dispose == BiObject.TYPE_FUNCTION) { o.dispose(); } else if (o instanceof Array) { for (var j = o.length - 1; j >= 0; j--) { p = o[j]; if (p && typeof p.dispose == BiObject.TYPE_FUNCTION) { p.dispose(); } } } }
            delete this[n];
        }
    }
};
...
_p.getProperty = function(sPropertyName) {
    ...
    if (typeof this[getterName] == BiObject.TYPE_FUNCTION)
...
_p.setProperty = function(sPropertyName, oValue) {
    ...
    if (typeof this[setterName] == BiObject.TYPE_FUNCTION)
        this[setterName](oValue);
    else throw new Error("No such property, " + sPropertyName);
};
_p.setProperties = function(oProperties) { for (var p in oProperties) { this.setProperty(p, oProperties[p]); } };
_p.setAttribute = function(sName, sValue, oParser) {
    var v, vv;
    if (sValue == String.BOOLEAN_TRUE)
        v = true;
    else if (sValue == String.BOOLEAN_FALSE)
        v = false;
    else if ((vv = parseFloat(sValue)) == sValue)
        v = vv;
    else v = sValue;
    this.setProperty(sName, v);
};
_p.getAttribute = function(sName) { return String(this.getProperty(sName)); };
_p.addXmlNode = function(oNode, oParser) {
    if (oNode.nodeType == 1)
        oParser.fromNode(oNode);
};

function _ClassStub() {}
_p = _ClassStub.prototype;
_p.addBundle = _p.attachToWindow = _p.dispose = _p.getAppearanceTag = Function.EMPTY;
BiAdf = BiFocusManager = BiEventManager = BiStringBundle = BiThemeManager = BiXmlLoader = _ClassStub;
if (typeof BiObject == "undefined")
    BiObject = new Function;

function BiBrowserCheck() { throw new Error("Cannot create instance of BiBrowserCheck."); };
_biExtend(BiBrowserCheck, BiObject, "BiBrowserCheck");
BiBrowserCheck.ie = /msie/i.test(navigator.userAgent);
BiBrowserCheck.moz = navigator.product == "Gecko";
BiBrowserCheck.platform = navigator.platform;
BiBrowserCheck.hta = BiBrowserCheck.ie && !window.external;
if (!(BiBrowserCheck.moz ? /rv\:(.+)[\);]/ : /MSIE\s+(.+)[\);]/).test(navigator.userAgent))
    throw new Error("Unable to detect Browser version.");
BiBrowserCheck.version = RegExp.$1;
BiBrowserCheck.versionNumber = parseFloat(BiBrowserCheck.version);
BiBrowserCheck.supported = (BiBrowserCheck.ie && BiBrowserCheck.versionNumber >= 5.5) || (BiBrowserCheck.moz && BiBrowserCheck.versionNumber >= 1.4);
...
_p._hrefCache = null;
_p._generic = true;
BiUri.prototype.getScheme = function() { return this._scheme; };
BiUri.prototype.getPath = function() { return this._path; };
BiUri.prototype.getDirPath = function() { return this._dirPath; };
BiUri.prototype.getHost = function() { return this._host; };
BiUri.prototype.getPort = function() { return this._port; };
BiUri.prototype.getFragment = function() { return this._fragment; };
BiUri.prototype.getUserInfo = function() { return this._userInfo; };
BiUri.regExps = { scheme: /^([^:]+)\:.+$/, user: /^([^@\/]+)@.+$/, host: /^([^:\/\?\#]+).*$/, port: /^:(\d+)/, path: /^([^\?#]*)/, dirPath: /^(.*\/)[^\/]*$/, fragment: /^[^#]*#(.*)$/, absUri: /^\w(\w|\d|\+|\-|\.)*:/i };
_p.toString = function() { return this.getHref(); };
_p.setHref = function(s) {
    ...
    if (this._generic)
        s = s.substring(this._scheme.length + 3);
...
_p._parsePathAndRest = function(s) {
    ...
    if (this._path == "" && (this._scheme == "file" || this._scheme == "http" || this._scheme == "https" || this._scheme == "ftp")) { this._path = "/"; }
    ...
    if (this._query != "") {
        ...
        for (i = 0; i < pairs.length; i++) {
            parts = pairs[i].split("=");
            try { name = decodeURIComponent(parts[0]); } catch (e) { name = parts[0]; }
            if (parts.length == 2) {
                try { value = decodeURIComponent(parts[1]); } catch (e) { value = parts[1]; }
            } else value = null;
...
_p._setRelative = function(s) {
    ...
    if (isAbsolute) { this.setHref(s); return; }
    ...
    this._fragment = "";
    this._params = {};
    if (s.charAt(0) == "/") { this._parsePathAndRest(s) } else this._parsePathAndRest(dirPath + s);
};
_p.getHref = function() {
...
BiEvent.prototype.getType = function() { return this._type; };
BiEvent.prototype.getTarget = function() { return this._target; };
BiEvent.prototype.getCurrentTarget = function() { return this._currentTarget; };
BiEvent.prototype.getBubbles = function() { return this._bubbles; };
_p.stopPropagation = function() { this._propagationStopped = true; };
...
_p.addEventListener = function(sType, fHandler, oObject) {
    ...
    if (!(key in ls)) { this._listenersCount++; }
...
_p.removeEventListener = function(sType, fHandler, oObject) {
    if (this._disposed || !(sType in this._listeners))
        return;
    var key = BiObject.toHashCode(fHandler) + (oObject ? BiObject.toHashCode(oObject) : String.EMPTY);
    if (key in this._listeners[sType]) { this._listenersCount--; }
    delete this._listeners[sType][key];
    if (Object.isEmpty(this._listeners[sType])) { delete this._listeners[sType]; }
};
_p.dispatchEvent = function(e) {
    if (this._disposed)
        return;
    if (typeof e == BiObject.TYPE_STRING) { e = new BiEvent(e); }
    e._target = this;
...
_p.hasListeners = function(sType) { return this._listenersCount > 0 && (sType == null || sType in this._listeners); };
...
BiXmlDocument.setNamespaces = function(oNode, oNamespaces) {
    if (oNode.nodeType != 9) { oNode = oNode.ownerDocument; }
    var sb = [];
    for (i in oNamespaces) { sb.push("xmlns:", i, "=\"", oNamespaces[i], "\" "); }
...
if (typeof ActiveXObject == "undefined" && typeof XMLHttpRequest != "undefined" && !XMLHttpRequest.____ALREADY_LOADED____) {
    (function() {
        ...
        _p.__defineGetter__("onreadystatechange", function() { return this._onreadystatechange; });
        BiXmlDocument._mozHasParseError = function(oDoc) { return !oDoc.documentElement || oDoc.documentElement.localName == "parsererror" && oDoc.documentElement.getAttribute("xmlns") == "http://www.mozilla.org/newlayout/xml/parsererror.xml"; };
        ...
        _p.__defineGetter__("xml", function() { return (new XMLSerializer).serializeToString(this); });
        _p.__defineGetter__("baseName", function() { var lParts = this.nodeName.split(":"); return lParts[lParts.length - 1]; });
        ...
        _p.selectSingleNode = function(sExpr) {
            var doc = this.nodeType == 9 ? this : this.ownerDocument;
            var nsRes;
            try { nsRes = doc.createNSResolver(this.nodeType == 9 ? this.documentElement : this); } catch (e) { nsRes = null; }
            var nsRes2;
            if (doc._selectionNamespaces) {
                nsRes2 = function(s) {
                    if (s in doc._selectionNamespaces)
        ...
        _p.__defineGetter__("xml", function() { var nv = (new XMLSerializer).serializeToString(this); return this.nodeName + "=\"" + nv.replace(/\"/g, "&quot;") + "\""; });
        var _textPrototype = Text.prototype;
        _textPrototype.__proto__ = { __proto__: _textPrototype.__proto__ };
        _p = _textPrototype.__proto__;
        _p.__defineGetter__("text", function() { return this.nodeValue; });
    })();
}
...
_p.getReuseWindow = function() { return this._reuseWindow; };
_p.setReuseWindow = function(b) { this._reuseWindow = b; };
_p.getNewWindow = function() { return this._newWindow; };
_p.setNewWindow = function(b) { this._newWindow = b; };
_p.getWindow = function() { return this._window || null; };
_p.getRootPath = function() { return this._rootPath; };
...
_p.getAdfPath = function() { return this._adfPath; };
...
_p.getArguments = function() { return this._arguments; };
_p.setArguments = function(a) {
    this._arguments = [];
    for (var i = 0; i < a.length; i++)
        this._arguments.push(String(a[i]));
};
_p.getTarget = function() { return this._target; };
_p.setTarget = function(s) { this._target = s; };
_p.setAccessibilityMode = function(b) { this._accessibilityMode = b; };
_p.getAccessibilityMode = function() { return this._accessibilityMode; };
_p.getFocusOnLoad = function() { return this._focusOnLoad; };
_p.setFocusOnLoad = function(b) { this._focusOnLoad = b; };
_p.getSupported = function() {
    var p;
    if (BiBrowserCheck.ie) {
        p = String(BiBrowserCheck.platform).toLowerCase();
        if (p != "win32" && p != "win64") { this._errorMessage = BiLauncher.IE_ERROR_PLATFORM; return false; }
        if (BiBrowserCheck.versionNumber < 5.5) { this._errorMessage = BiLauncher.IE_ERROR_VERSION; return false; }
        return true;
    } else if (BiBrowserCheck.moz) {
        if (BiBrowserCheck.versionNumber < 1.4) { this._errorMessage = BiLauncher.GECKO_ERROR_VERSION; return false; }
        return true;
    }
    this._errorMessage = BiLauncher.NOT_SUPPORTED_ERROR;
    return false;
};
_p.getErrorMessage = function() { return this._errorMessage; };
_p.getHasError = function() { return this._errorMessage != ""; };
_p.launch = function(sAdfPath, oArgs) {
    ...
    if (n)
        this._focusOnLoad = n.text != "false";
    if (!bUseCurrentWindow) {
        ...
        if (!w) { this._errorMessage = BiLauncher.POPUP_BLOCKER_QUESTION; return false; }
...
BiLauncher._toPixel = function(s, sAxis) {
    if (String(s).indexOf("%") != -1) { var n = Number(s.replace(/\%/g, "")); return n / 100 * (sAxis == "x" ? screen.availWidth : screen.availHeight); }
    return s;
};
BiLauncher._getAttr = function(el, name, def, tp) {
    ...
    if (tp)
        return BiLauncher._toPixel(res, tp);
    return res;
};
...
_p.hasKey = function(key) { return this._hash.hasOwnProperty(key); };
_p.getKeys = function() {
    var res = [];
...
_p.isEmpty = function() { return this._count == 0; };
_p._recreate = function() {
    var tmp = {};
    for (var k in this._hash) { if (this._hash.hasOwnProperty(k)) { tmp[k] = this._hash[k] } }
    this._hash = tmp;
...
BiXmlResourceParser.prototype.getAutoNameMapping = function() { return this._autoNameMapping; };
BiXmlResourceParser.prototype.setAutoNameMapping = function(v) { this._autoNameMapping = v; };
_p._rootNode = null;
BiXmlResourceParser.getClassFromUri = function(oUri) { return BiXmlResourceParser.getClassFromDocument(BiXmlLoader.load(oUri)); };
BiXmlResourceParser.getClassFromDocument = function(oDoc) { return BiXmlResourceParser.getClassFromNode(oDoc.documentElement); };
...
BiXmlResourceParser.prototype.setRootNode = function(v) { this._rootNode = v; };
_p.getRootNode = function() {
    if (this._rootNode) { return this._rootNode; } else {
        if (this.getLoaded())
...
_p.fromNode = function(oNode) {
    ...
    var c;
    if (id && (c = this._componentsById.item(id))) { if (c.getDisposed()) { this._removeObject(c); } else { return c; } }
    var tagName = oNode.localName || oNode.baseName;
    var o;
    var constr = window["Bi" + tagName] || window[tagName];
...
_p.processAttributes = function(o, oNode) {
    ...
    for (var i = 0; i < l; i++) {
        ...
        if (name == "xmlns" || name.indexOf(":") > 0)
            continue;
        if (name.indexOf(".") > 0) {
            parts = name.split(".");
            className = parts[0];
            setterName = "set" + parts[1].capitalize();
            constr = window["Bi" + className] || window[className];
            if (typeof constr == "function") {
                if (typeof constr[setterName] == "function") { constr[setterName](o, value); } else throw new Error("No such attached property \"" + name + "\"");
            } else throw new Error("No such class: \"" + className + "\"");
        } else o.setAttribute(name, value, this);
        if (name == "id")
            this._addObject(o, value);
    }
};
...
_p.getComponentById = function(sId) {
    var o = this._componentsById.item(sId);
    if (o) {
        if (o.getDisposed())
            this._removeObject(o);
        else return o;
    }
    if (this.getLoaded()) {
...
_p.getLoaded = function() { return this._rootNode != null || BiXmlLoader.prototype.getLoaded.call(this); };
_p.dispose = function() {
    if (this.getDisposed()) return;
    BiXmlLoader.prototype.dispose.call(this);
    var vs = this._componentsById.getValues();
    for (var i = 0; i < vs.length; i++) { this._removeObject(vs[i]); }
    this._componentsById.dispose();
...
_p.contains = function(o) { return o && this._items.hasKey(BiObject.toHashCode(o)); };
...
function BiTimerManager() {
    if (_biInPrototype) return;
    if (BiTimerManager._singleton)
        return BiTimerManager._singleton;
    BiObject.call(this);
    this._timers = new BiSet;
...
_p.add = function(oTimer) { this._timers.add(oTimer); };
_p.remove = function(oTimer) { this._timers.remove(oTimer); };
_p.dispose = function() {
    if (this._disposed)
        return;
    BiObject.prototype.dispose.call(this);
    var vs = this._timers.getValues();
...
BiTimer.prototype.getEnabled = function() { return this._enabled; };
BiTimer.prototype.setEnabled = function(v) { this._enabled = v; };
BiTimer.prototype.getInterval = function() { return this._interval; };
...
_p._version = "3.0";
BiApplication.prototype.getVersion = function() { return this._version; };
...
_p._onunload = function() { application.dispose(); };
_p._uri = new BiUri(window.location.href);
_p._uriParams = new BiUri(window.location.href);
_p._systemRootPath = new BiUri(window.location.href, "./");
_p.getPath = function() { return this._systemRootPath; };
...
BiApplication.prototype.getProgressStatus = function() { return this._progressStatus; };
BiApplication.prototype.setProgressStatus = function(v) { this._progressStatus = v; };
BiApplication.prototype.getWindow = function() { return this._window; };
BiApplication.prototype.getAdf = function() { return this._adf; };
BiApplication.prototype.getUri = function() { return this._uri; };
_p._accessibilityMode = false;
BiApplication.prototype.getAccessibilityMode = function() { return this._accessibilityMode; };
...
BiApplication.prototype.getAccessibilityDescription = function() { return this._accessibilityDescription; };
BiApplication.prototype.setAccessibilityDescription = function(v) { this._accessibilityDescription = v; };
_p._autoNameMapping = false;
...
_p._onAdfLoaded = function() {
    ...
    this.dispatchEvent("progressstatus");
    if (this._adf.getError()) { this._reportError(this._getString("ApplicationAdfLoadError"), this._getString("ApplicationAdfLoadErrorDetails", this._uriParams.getParam("Adf"), this._adf.getXmlHttp().status, this._adf.getXmlHttp().statusText)); } else {
        this._adf._interpret();
        if (BiBrowserCheck.ie)
            BiTimer.callOnce(this._loadResources, 1, this);
        else application._loadResources();
    }
};
_p.getResourceLoader = function() { return this._resourceLoader; };
...
_p.getComponentById = function(sId) {
    if (this._adf && this._adf.getXmlResourceParser())
        return this._adf.getXmlResourceParser().getComponentById(sId);
    return null;
};
_p._loadResources = function() {
    ...
    for (var i = 0; i < this._defaultPackages.length; i++) {
        ...
        for (var j = 0; j < files.length; j++) { this._resourceLoader.addResource("script", new BiUri(systemRootPath, files[j])); }
    }
...
_p._addAccessibilityPackage = function() {
    if (this._accessibilityMode && !this._508Added) {
        var files = this.getPackage("Accessibility");
        var systemRootPath = this.getPath();
        for (var j = 0; j < files.length; j++) { this._resourceLoader.addResource("script", new BiUri(systemRootPath, files[j])); }
        this._508Added = true;
    }
...
_p._onResourcesLoaded = function(e) {
    ...
    if (this._useTimersWorkAround) { BiTimer.callOnce(this._onResourcesLoaded2, 1, this); } else this._onResourcesLoaded2();
};
_p._onResourcesLoaded2 = function() {
    this._adf.parseXmlResources();
    this.dispatchEvent("resourcesready");
    if (this._useTimersWorkAround) { BiTimer.callOnce(this._onResourcesLoaded3, 1, this); } else this._onResourcesLoaded3();
};
_p._onResourcesLoaded3 = function() {
    ...
    if (this._useTimersWorkAround) { BiTimer.callOnce(this._onResourcesLoaded4, 1, this); } else { application._onResourcesLoaded4(); }
};
...
_p.dispose = function() {
    ...
    if (BiBrowserCheck.ie)
        window.detachEvent("onunload", this._onunload);
...
_p.getTheme = function() { return this.getThemeManager().getDefaultTheme(); };
BiApplication.prototype.getInactivityTimeout = function() { return this._inactivityTimeout; };
BiApplication.prototype.setInactivityTimeout = function(n) {
    n = Number(n) || 0;
...
_p.add = function(oChild, oBefore, bAnonymous) {
    ...
    oChild._parent = this;
    if (this._created)
        oChild._addHtmlElementToParent(this, oBefore);
};
...
_p.removeAll = function() {
    ...
    for (var i = 0; i < l; i++) {
        var c = cs[i];
        if (c._anonymous) { this._children.push(c); } else { c.dispose(); }
    }
};
...
_p.setParent = function(p) { if (p != null && p != this._parent) { p.add(this); } else if (this._parent != null) { this._parent.remove(this); } };
...
_p.setChildren = function(cs) { this.removeAll(); if (cs != null) { for (var i = 0; i < cs.length; i++) { this.add(cs[i]); } } };
_p.setStyleProperty = function(sProp, sValue) {
    if (sValue == String.EMPTY) { delete this._style[sProp]; } else { this._style[sProp] = sValue; }
    if (this._created)
...
_p.getBackColor = function() { return this.getStyleProperty("backgroundColor"); };
_p.setVisible = function(bVisible) {
    ...
    if (this._lazyCreate && !this._created && bVisible) {
        ...
        if (p && p._created)
            this._addHtmlElementToParent(p, this.getNextSibling(), true);
    }
};
...
_p.getIsVisible = function() {
    ...
    if (BiBrowserCheck.ie) {
        while (el.tagName != "BODY") {
            if (el.currentStyle.visibility == BiComponent.STYLE_INHERIT) { el = el.parentNode; continue; }
            return el.currentStyle.visibility == BiComponent.STYLE_VISIBLE;
        }
    } else { var compStyle = this._document.defaultView.getComputedStyle(el, null); return compStyle.visibility != BiComponent.STYLE_HIDDEN && compStyle.display != BiComponent.STYLE_NONE; }
    return true;
};
_p.setOpacity = function(n) {
    ...
    if (this._opacity != n) {
        this._opacity = n;
        if (BiBrowserCheck.ie) { this.setStyleProperty("filter", this._getIeFilter()); } else {
            if (n == 1)
...
BiComponent.prototype.getOpacity = function() { return this._opacity; };
...
_p.invalidateParentLayout = function(sHint) {
    ...
    var p = this._parent;
    if (p)
        p.invalidateChild(this, sHint);
};
_p.invalidateLayout = function() {
    this._invalidLayout = true;
    if (this._created)
        BiComponent.enqueueLayout(this);
};
_p.invalidateChild = function(oChild, sHint) {
    if (sHint == null || sHint == BiComponent.STRING_SIZE)
...
BiComponent.enqueueLayout = function(c) {
    var hc = c.toHashCode();
    if (this._layoutQueue.hasKey(hc))
        return;
...
_p.layoutComponent = function() { var p; if (this._created && (p = this._parent)) { p.layoutChild(this); } };
_p.layoutChild = function(oChild) { var sizeChanged = this._layoutChild(oChild); if (sizeChanged) { oChild.invalidateLayout(); } };
_p.layoutAllChildren = function() {
    var cs = this._children;
    var l = cs.length;
    for (var i = 0; i < l; i++) { cs[i].layoutComponent(); }
...
_p._layoutChild = function(oChild) {
    ...
    if (oChild._left != null) { x = oChild._left; if (oChild._right != null) { cw = this.getClientWidth();
        ...
        if (oChild._width != null)
            ...
            if (BiBrowserCheck.ie && oChild._measuredHeight == 0 && oChild._created) { oChild._element.runtimeStyle.display = String.EMPTY; }
            w = oChild.getWidth();
        }
...
_p._layoutChild2 = function(c, x, y, w, h, bInvalidate) {
    var wChanged = false;
    var hChanged = false;
    var componentMoved = (c._clientLeft != null && x != c._clientLeft) ||
        (c._clientTop != null && y != c._clientTop);
    c._clientLeft = x;
    c._clientTop = y;
    ...
    if (wChanged || hChanged) {
        ...
        if (bInvalidate) { c.invalidateLayout(); }
        return true;
    }
    return false;
};
_p.getLeft = function() {
    if (this._created && this._parent) { BiComponent.flushLayoutComponent(this); return this._element.offsetLeft; }
    return this._left;
};
_p.getRight = function() {
    if (this._created && this._parent) { BiComponent.flushLayoutComponent(this); return this._parent.getClientWidth() - this.getLeft() - this.getWidth(); }
    return this._right;
};
_p.getTop = function() {
    if (this._created && this._parent) { BiComponent.flushLayoutComponent(this); return this._element.offsetTop; }
    return this._top;
};
_p.getBottom = function() {
    if (this._created && this._parent) { BiComponent.flushLayoutComponent(this); return this._parent.getClientHeight() - this.getTop() - this.getHeight(); }
    return this._bottom;
};
...
_p.getClientWidth = function() {
    if (this._created) { BiComponent.flushLayoutComponent(this); return this._element.clientWidth; }
    throw new Error("Visual property on non created component");
};
_p.getClientHeight = function() {
    if (this._created) { BiComponent.flushLayoutComponent(this); return this._element.clientHeight; }
    throw new Error("Visual property on non created component");
};
_p.getIsEnabled = function() {
    if (!this.getEnabled()) { return false; }
    if (this._created) {
        if (BiBrowserCheck.ie) { return !this._element.isDisabled; } else {
            var el = this._element;
            while (el) {
        ...
        var p = this.getParent();
        if (p)
            return p.getIsEnabled();
        return true;
    }
};
BiComponent.prototype.getCreated = function() { return this._created; };
...
_p._setHtmlProperties = function() { var el = this._element; var hp = this._htmlProperties; for (var p in hp) { el[p] = hp[p]; } };
_p._setHtmlAttributes = function() {
    var el = this._element;
    var ha = this._htmlAttributes;
    for (var n in ha)
        el.setAttribute(n, ha[n]);
};
_p._addHtmlElementToParent = function(oParent, oBefore) {
    if (this._lazyCreate && !this.getVisible())
        return;
    if (!this._created)
        this._create(oParent._document);
    var beforeElement = oBefore ? oBefore._element : null;
    while (beforeElement && beforeElement.parentNode != oParent._element)
        beforeElement = beforeElement.parentNode;
    if (beforeElement)
        oParent._element.insertBefore(this._element, beforeElement);
    else oParent._element.appendChild(this._element);
    this._created = true;
    this._createChildren();
    this.invalidateParentLayout();
    if (oParent.getHtmlProperty("disabled") && !this.getHtmlProperty("disabled")) { this._setEnabled(false); }
    this.dispatchEvent(BiComponent._createEvent);
};
_p._removeHtmlElement = function(oChild) { oChild._removeHtmlElementFromParent(this); };
_p._removeHtmlElementFromParent = function(oParent) { if (this._created && (oParent && oParent._created && oParent._element) && !(application && application._disposed)) { oParent._element.removeChild(this._element); } };
_p.dispose = function() {
    if (this._disposed) return;
    BiComponent._layoutQueue.remove(this.toHashCode());
    BiEventTarget.prototype.dispose.call(this);
    if (this._parent) {
        try { this._parent.remove(this); } catch (e) {}
        delete this._parent;
    }
    var el = this._element;
    if (el) {
        ...
        if (BiBrowserCheck.ie) { el.removeAttribute("_biComponent"); }
        el = null;
    }
...
_p._create = function(oDocument) {
    ...
    if (!el)
        throw new Error("BiInlineComponent, could not find element in page");
    el._biComponent = this;
    this._setHtmlProperties();
    this._setCssProperties();
    if (BiBrowserCheck.ie)
...
_p._moveTo = function(nLeft, nTop) {
    try { this._window.moveTo(nLeft, nTop); } catch (ex) {}
}
_p._resizeTo = function(nWidth, nHeight) {
    try { this._window.resizeTo(nWidth, nHeight); } catch (ex) {}
}
_p.setLeft = function(nLeft) { this._moveTo(nLeft, this.getTop()); };
_p.setRight = function(nRight) { this.setLeft(this._window.screen.width - this.getWidth() - nRight); };
_p.getScreenLeft = _p.getLeft = function() {
    if (BiBrowserCheck.ie)
        return this._window.screenLeft - this._getInsets().left;
    else return this._window.screenX;
};
_p.getRight = function() { return this._window.screen.width - this.getLeft() - this.getWidth(); };
_p.setTop = function(nTop) { this._moveTo(this.getLeft(), nTop); };
_p.setBottom = function(nBottom) { this.setTop(this._window.screen.height - this.getHeight() - nBottom); };
_p.setLocation = function(nLeft, nTop) { this._moveTo(nLeft, nTop); };
_p.getScreenTop = _p.getTop = function() {
    if (BiBrowserCheck.ie)
        return this._window.screenTop - this._getInsets().top;
    else return this._window.screenY;
};
_p.getBottom = function() { return this._window.screen.height - this.getTop() - this.getHeight(); };
_p.setWidth = function(nWidth) {
    if (BiBrowserCheck.ie)
        this._resizeTo(nWidth, this.getHeight());
...
_p.setHeight = function(nHeight) {
    if (BiBrowserCheck.ie)
        this._resizeTo(this.getWidth(), nHeight);
    else this._window.outerHeight = nHeight;
};
_p.setSize = function(nWidth, nHeight) { this._resizeTo(nWidth, nHeight); };
_p.getHeight = function() {
    if (BiBrowserCheck.ie)
        return this._getSize().height;
    else return this._window.outerHeight;
};
_p.getInsetLeft = function() { return this._getInsets().left + BiComponent.prototype.getInsetLeft.call(this); };
_p.getInsetRight = function() { return this.getWidth() - this.getInsetLeft() - this.getClientWidth(); };
_p.getInsetTop = function() { return this._getInsets().top + BiComponent.prototype.getInsetTop.call(this); };
_p.getInsetBottom = function() { return this.getHeight() - this.getInsetTop() - this.getClientHeight(); };
_p.getClientLeft = function() { return -this.getInsetLeft(); };
_p.getClientTop = function() { return -this.getInsetTop(); };
_p._getScreenClientArea = function() {
    if (BiBrowserCheck.ie)
        return { left: this._window.screenLeft, top: this._window.screenTop, width: this._element.clientWidth, height: this._element.clientHeight };
    else { var bo = this._document.getBoxObjectFor(this._document.documentElement); return { left: bo.screenX, top: bo.screenY, width: this._element.clientWidth, height: this._element.clientHeight }; }
};
_p.setZIndex = _p.getZIndex = _p.setVisible = _p.getVisible = function() { throw new Error("Not supported"); };
_p.getTopLevelComponent = function() { return this; };
_p.getIsVisible = function() { return true; };
_p.getCaption = function() { return this._window.document.title; };
_p.setCaption = function(sTitle) { this._window.document.title = sTitle; };
_p.getFullScreen = function() { throw new Error("Not yet implemented"); };
_p.getResizable = function() { throw new Error("Not yet implemented"); };
_p.close = function() { this._window.close(); };
_p.print = function() { this._window.print(); };
_p.isFocusRoot = function() { return true; };
_p.getFocusRoot = function() { return this; };
_p.getActiveComponent = function() {
    if (this._activeComponent && this._activeComponent.getDisposed())
        this._activeComponent = null;
    return this._activeComponent;
};
BiApplicationWindow.prototype.getAcceptButton = function() { return this._acceptButton; };
BiApplicationWindow.prototype.setAcceptButton = function(v) { this._acceptButton = v; };
BiApplicationWindow.prototype.getCancelButton = function() { return this._cancelButton; };
BiApplicationWindow.prototype.setCancelButton = function(v) { this._cancelButton = v; };
...
_p._create = function(oWindow) {
    ...
    var cs = this._children;
    var l = cs.length;
    for (var i = 0; i < l; i++) { cs[i]._addHtmlElementToParent(this); }
};
_p.dispose = function() {
    if (this.getDisposed()) return;
...
_p._ondefaultbuttonkeydown = function(e) {
    ...
    var t = e.getTarget();
    switch (e.getKeyCode()) {
        case BiKeyboardEvent.ENTER:
            if (this._acceptButton && this._acceptButton.getEnabled() && !t._acceptsEnter && !e.getAltKey()) {
                this._acceptButton.dispatchEvent("action");
                if (this._acceptButton.getCommand()) { this._acceptButton.getCommand().execute() }
            }
            break;
        case BiKeyboardEvent.ESC:
            if (this._cancelButton && this._cancelButton.getEnabled() && !t._acceptsEsc) {
                this._cancelButton.dispatchEvent("action");
                if (this._cancelButton.getCommand()) { this._cancelButton.getCommand().execute() }
            }
            break;
    }
};
_p._getSize = function() {
    ...
    if (this._diff == null)
        this._diff = { width: Number(this._insetLeft) + Number(this._insetRight), height: Number(this._insetTop) + Number(this._insetBottom) };
    ...
    var diff = { width: oldInnerSize.width - newInnerSize.width + this._diff.width, height: oldInnerSize.height - newInnerSize.height + this._diff.height };
    this._resizeTo(oldInnerSize.width + diff.width, oldInnerSize.height + diff.height);
    this._diff = diff;
    return { width: oldInnerSize.width + diff.width, height: oldInnerSize.height + diff.height };
};
_p._getInnerSize = function() { return { width: this.getClientWidth(), height: this.getClientHeight() }; };
_p._getInsets = function() {
    if (BiBrowserCheck.ie) {
        ...
        if (this._insets == null)
            this._insets = { left: Number(this._insetLeft), top: Number(this._insetTop) };
        ...
        var res = { left: newScreenLeft - oldScreenLeft + this._insets.left, top: newScreenTop - oldScreenTop + this._insets.top };
...
BiFont.prototype.getBold = function() { return this._bold; };
BiFont.prototype.setBold = function(v) { this._bold = v; };
BiFont.prototype.getItalic = function() { return this._italic; };
BiFont.prototype.setItalic = function(v) { this._italic = v; };
BiFont.prototype.getUnderline = function() { return this._underline; };
BiFont.prototype.setUnderline = function(v) { this._underline = v; };
BiFont.prototype.getStrikeout = function() { return this._strikeout; };
BiFont.prototype.setStrikeout = function(v) { this._strikeout = v; };
BiFont.prototype.getName = function() { return this._name; };
BiFont.prototype.setName = function(v) { this._name = v; };
_p.paintFont = function(oComponent) {
    if (this._name)
        oComponent.setStyleProperty("fontFamily", this._name);
    if (this._size != null)
        oComponent.setStyleProperty("fontSize", this._size + "px");
    if (this._bold != null)
        oComponent.setStyleProperty("fontWeight", this._bold ? "bold" : "normal");
    if (this._italic != null)
        oComponent.setStyleProperty("fontStyle", this._italic ? "italic" : "normal");
    var td = null;
    if (this._underline == false && this._strikeout == false)
        td = "none";
    else if (this._underline != null || this._strikeout != null) { td = (this._underline ? "underline" : "") + (this._strikeout ? " line-through" : ""); }
    if (td != null)
...
bindows = {
    loadGaugeIntoDiv: function(sUri, divId) {
        ...
        var doc = xmlHttp.responseXML;
        doc.loadXML(s);
        var gaugeClass = BiXmlResourceParser.getClassFromDocument(doc);
        var gauge = new gaugeClass();
        gauge.setVisible(false);
        var c = new BiInlineComponent(divId);
        c.add(gauge);
        application._window.add(c);
        BiTimer.callOnce(function() { gauge.setVisible(true); })
        var g = { __gauge: gauge }
        var ids = gauge._xmlResourceParser._componentsById.getKeys();
...
(function() {
    ...
    var script;
    for (var i = 0; i < scripts.length; i++) { if (scripts[i].src.indexOf(packagedName) != -1) { script = scripts[i]; break; } }
    var s = script.src;
    var base = s.substring(0, s.indexOf(packagedName));
    var js = "";
    if (BiBrowserCheck.ie) js = bindows._syncRequest(base + "gauge2.ie.js").responseText;
    if (BiBrowserCheck.moz) js = bindows._syncRequest(base + "gauge2.moz.js").responseText;
