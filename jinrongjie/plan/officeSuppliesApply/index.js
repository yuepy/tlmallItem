            ...
            if ($(this).next().children('.zdm').length == 2 && $(this).next().children('.zdn').length == 0) {
              var _this = this;$(this).next().children(".zdm").eq(1).find("tr").each(function (i) {
                if ($(this)[0].style.display !== 'none') {
                  var arr2 = [];arr2.push({ num: $(this).children("td").eq(0).find("input").prop("value"), idInput: $(this).children("td").eq(0).find("input").prop("id"), no: i, type: 'card', idButton: $(_this).next().children(".zdm").eq(0).find("tr").eq(i).children("td").eq(0).find("button")[0].getAttribute('onClick').match(/field\d+_\d+/) });
