(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
        ...
        var elem1 = elem.contentDocument.getElementById("oTable1");if ($(elem1).find("#frame1").length > 0) {
          var elem2 = $(elem1).find("#frame1")[0].contentDocument.body;if ($(elem2).find("#deeptree").length > 0) {
            data.status.push("1");var tree = $(elem2).find("#deeptree")[0];$(tree).find(".webfx-tree-item").each(function () {
              if ($(this).text().trim() !== '' && $(this)[0].style.display !== 'none' && $(this).parent()[0].style.display !== 'none' && $(this).parent().parent()[0].style.display !== 'none' && $(this).parent().parent().parent()[0].style.display !== 'none' && $(this).parent().parent().parent().parent()[0].style.display !== 'none') {
                // var container = [];
                // container.push($(this).parents(".webfx-tree-container"));
                // var ies = container.every(function (container) {
                //   return container[0].style.display !== 'none';
                // }); 
                //console.log(ies）
                var arr = [];var imgth = $(this).find("img").length;var imgEle = $(this)[0].querySelectorAll("img");for (var i = 0; i < imgth; i++) {
                  if (/plus/.test(imgEle[i].src) && /global/.test(imgEle[i + 1].src)) {
                    ...
                    arr.push({ status: 'bottom',
                      text: $(this).find("a").text(), id: $(this).find("a")[0].id, length: $(this).find("img").length, imgId: $(this).find("img").eq(i)[0].id });data.title.push(arr);break;
    ...
    getTemplate_uiControl36_Ik9Qso: function () {
