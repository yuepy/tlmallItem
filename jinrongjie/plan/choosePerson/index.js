...
(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control31_MQw3uc: function (elem) {
      console.log(elem);var data = { search: { inputValue: [] }, status: [], title: [], content1: [], content2: [], content3: [], thead: ['姓名', '岗位', "部门"], display_status: [] }; //style={{color:'rgb(194,154,42)'}}
      ...
      if (data.eventType == 'down') {
        //debugger;
        var elem1 = elem.contentDocument.getElementById("oTable1");var elem3 = $(elem1).find("#frame2")[0].contentDocument.body;var Tbody = $(elem3).find("table").children("tbody")[0];
        var tr0 = $(Tbody).children("tr").eq(0)[0]; // console.log($(tr0).find("input"));
        if ($(tr0).children("td").length > 0 && $(tr0).find("input").length > 0) {
