(function(win, ysp) {
    ysp.runtime.Model.extendLoadingModel({
                        ...
                        if ($(this)[0].style.display !== 'none') {
                            $(this).children("td").each(function() {
                                ...
                                if ($(this)[0].className == 'zdm' && /签字意见/.test($(this).text()) && $(this).next()[0].className == 'zdn' && ($(this).next().children().length == 0 || $(this).next()[0].children.length > 0 && $(this).next()[0].children[0].tagName == 'BR')) {
                                    ...
                                    arr2.push({
                                        text: $(this).text().replace(/\s/ig, "").trim(),
                                        type: 'tit_yell2'
                                    });
        ...
        getData_control195_EFbUCS: function(elem) {
            if (!elem) {
                return;
            }
            var data = { title: [], content: [] };
            if ($(elem).find("#requestlogappednDiv").length > 0 && $(elem).find("#requestlogappednDiv").children().length > 0) {
        ...
        doAction_uiControl177_K8Czts: function(data, elem) {
            if (data.eventType == 'block') {
                debugger;
                var val = parseInt(data.dataCustom) + 1;
                if ($(elem).find("#requestlogappednDiv").length > 0 && $(elem).find("#requestlogappednDiv").children().length > 0) {
                    var elem = $(elem).children("div").eq(0).find("tbody")[0];
                } else {
                    var elem = $(elem).children("#mainWFHead").eq(0).find("tbody")[0];
        ...
        getTemplate_uiControl177_K8Czts: function() {
