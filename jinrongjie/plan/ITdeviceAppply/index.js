...
(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control165_NdPmnp: function getData_control165_NdPmnp(elem) {},
    doAction_uiControl142_GUkTUk: function doAction_uiControl142_GUkTUk(data, elem) {
    ...
    getData_control166_QnAye0: function (elem) {
      if (!elem) {
        return;
      }var data = { header: { title: [], number: [] }, base_lc_info: { title: [], content: [], degree: [], miji: { content: [], id: [] } }, base_info: { content: [] }, related_document: [], related_process: [], file: [], state: [] };if ($(elem).find('#field15187span').length > 0 || $(elem).find('#field15188span').length > 0) {
        if ($(elem).find('#field15187span').find("a").length > 0 || $(elem).find('#field15188span').find("a").length > 0) {
        ...
        var elem1 = $(elem).children("table").eq(0)[0];if ($(elem1).find("tr").length > 0) {
          ...
          if ($(this)[0].style.display !== 'none') {
            $(this).children("td").each(function () {
              if ($(this).next().find('img').length == 1 && $(this).next().find('img').attr('src').indexOf('BacoError') !== -1) {
                var arr2 = [];if ($(this)[0].className == 'zdm' && /签字意见/.test($(this).text())) {
                  arr2.push({ text: $(this).text().replace(/\s/ig, "").trim(), type: 'tit_yell2', mark: '1' });
                ...
                //判断附件-------------------------------------
                    ...
                    var arr3 = [];if ($(this).children("table").find("a").length > 0) {
                    ...
                    }arr2.push({ text: arr3, size: arr4, type: 'fujian', mark: '1' });
                  } //判断select-------------------------------------
                      ...
                      var arr4 = [];$(this).children("select").children("option").each(function () {
                        ...
                        arr2.push({ text: $(this).children("input").prop('value'), type: 'input', id: $(this).children("input").prop('id'), mark: '1' });
                          ...
                          arr2.push({ text: $(this).children("span").children("a").text(), type: 'a', mark: '1' });
                        } else if ($(this).children("button").length == 0 && $(this).children("span").length == 1 && $(this).children("input").length == 1 && $(this).children("input")[0].type == 'hidden') {
                          arr2.push({ text: $(this).children("span").text(), type: 'a', mark: '1' });
                        } //判断textarea-------------------------------------
                        else if ($(this).children("textarea").length == 1) {
                            arr2.push({ text: $(this).children("textarea").prop('value'), type: 'textarea', id: $(this).children("textarea")[0].id, mark: '1' });
                          } //判断button-------------------------------------
                              ...
                              if ($(this).children("button").next("span").length > 0 && $(this).children("button").next("span").children("a").length > 0) {
                                $(this).children("button").each(function () {
                                  var arr = [];$(this).next("span").children('a').each(function () {
                                    arr.push($(this).text());
                                  });arr2.push({ text: arr, type: 'button', id: $(this)[0].getAttribute('onClick').match(/field\d+/), mark: '1' });
                                });
                              } else {
                                $(this).children("button").each(function () {
                              ...
                              arr2.push({ text: $(this).find("button").text(), type: 'button2', id: $(this).children('span').eq(0).children("button").attr('id'), mark: '1' });
                            } //判断意见-------------------------------------
                            else if ($(this)[0].className == 'zdn' && $(this).children().length == 0 || $(this)[0].children.length == 1 && $(this)[0].children[0].tagName == 'BR') {
                                arr2.push({ text: $(this)[0].innerHTML, type: 'suggest', mark: '1' });
                              } else if ($(this).find(".cke_editor").length > 0) {
                                arr2.push({ text: $(this).find(".cke_editor").find('iframe')[0].contentDocument.body.innerHTML, type: 'suggest_final', mark: '1' });
                              }
              } else {
                var arr2 = [];if ($(this)[0].className == 'zdm' && /签字意见/.test($(this).text())) {
                ...
                //               var useridReg = /userid=[0-9]{1,}/;
                //               var userid = check_form.match(useridReg)[0] && check_form.match(useridReg)[0].split("=")[1];
                //               var subId = elem.querySelector('#subId').value;
                //               var secId = elem.querySelector('#secId').value;
                //               var mainId = elem.querySelector('#mainId').value;
                //               user.userid = userid;
                //               user.subId = subId;
                //               user.secId = secId;
                //               user.mainId = mainId;
    ...
    doAction_uiControl143_8mx7V2: function (data, elem) {
      if (data.eventType == 'deleteFile') {
        ...
        var id = data.dataCustom.id;var val = data.dataCustom.value;var elem2 = $(elem).children("table").eq(1)[0];var tbody2 = $(elem).children("table").eq(1).children("tbody")[0];$(tbody2).find("select").each(function () {
          if ($(this)[0].id == id) {
            $(this).children("option").each(function () {
              if ($(this).text() == val) {
                $(this)[0].selected = 'true';$(this)[0].parentElement.dispatchEvent(new Event('change'));$(this)[0].parentElement.dispatchEvent(new Event('blur'));
              }
            });
          }
        });
        ...
        elem.ownerDocument.defaultView.eval(elem.querySelectorAll('.Browser')[2].onclick());
    ...
    doAction_uiControl144_TWZQiZ: function doAction_uiControl144_TWZQiZ(data, elem) {
      if (data.eventType == 'deleteFile') {
    ...
    getTemplate_uiControl144_TWZQiZ: function getTemplate_uiControl144_TWZQiZ() {
    ...
    getData_control168_AgTWpX: function getData_control168_AgTWpX(elem) {
      if (!elem) {
        return;
      }var data = [];data.push(elem.textContent);return data;
    },
    doAction_uiControl145_940azM: function doAction_uiControl145_940azM(data, elem) {
    ...
    getData_control335_HM34pl: function getData_control335_HM34pl(elem) {
      if (!elem) {
        return;
    ...
    doAction_uiControl335_0Upmhz: function doAction_uiControl335_0Upmhz(data, elem) {},
    getTemplate_uiControl335_0Upmhz: function getTemplate_uiControl335_0Upmhz() {
