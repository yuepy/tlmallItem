(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control66_fMMVCn: function (elem) {
      var data = { header: { title: [], number: [] }, base_lc_info: { title: [], content: [], degree: [], miji: [] }, base_info: { content: [] }, related_document: [], related_process: [], file: [] }; //附件-----------
                ...
                if ($(this).find("input").length !== 3 && $(this)[0].className == 'zdn' && i == tdl) {
                  data.base_lc_info.content.push($(this).text());
                    ...
                    $(this).children("select").children("option").each(function () {
                      if ($(this).text().trim() !== '') {
                        var arr = [];arr.push($(this).text().trim());data.base_lc_info.miji.push(arr);
                      }
                    });
                  }
              });
    ...
    }, doAction_uiControl63_9ffpTW: function (data, elem) {
