    ...
    ysp.runtime.Model.extendLoadingModel({
        getData_control108_46cZtf: function(elem) {
            ...
            if ($(elem).children("table").length == 2) {
                        ...
                        if ($(this).children("td").length == 1 && $(this).children("td")[0].className == 'zdm') {
                            ...
                            $(this).children("td").each(function(i) {
                                if ($(this).text().indexOf('紧急程度') !== -1 && $(this).next('td').children().length == 0) {
                                    ...
                                    var arrn = [];
                                    arrn.push($(this).text().trim());
                                    data.base_lc_info.degree.push(arrn);
                ...
                var elem2 = $(elem).children("table").eq(1)[0];
                ...
                var tbody2 = $(elem).children("table").eq(0).children("tbody")[0];
                $(tbody2).children("tr").each(function() {
                    //var arr1 = [];
                    $(this).children("td").each(function() {
                        var arr2 = []; //判断标题-------------------------------------
        ...
        doAction_uiControl93_sBQFT0: function(data, elem) {
            if (data.eventType == 'inputBlur') {
                ...
                var elem2 = $(elem).children("table").eq(1)[0];
                var tbody2 = $(elem).children("table").eq(1).children("tbody")[0];
                $(tbody2).find("input").each(function() {
                    if ($(this)[0].id == id) {
                        $(this)[0].value = val;
                    }
                });
            ...
            if (data.eventType == 'textarea1') {
                ...
                var elem2 = $(elem).children("table").eq(1)[0];
                var tbody2 = $(elem).children("table").eq(1).children("tbody")[0];
                $(tbody2).find("textarea").each(function() {
                    if ($(this)[0].id == id) {
            ...
            if (data.eventType == 'select') {
                ...
                var elem2 = $(elem).children("table").eq(1)[0];
                var tbody2 = $(elem).children("table").eq(1).children("tbody")[0];
                $(tbody2).find("select").each(function() {
                    if ($(this)[0].id == id) {
            ...
            if (data.eventType == 'textarea') {
                ...
                if ($(elem).find('.cke_editor').length > 0) {
                    var body = $(elem).find('.cke_editor').find("iframe")[0].contentDocument.body;
                    body.innerHTML = val;
                }
            }
            if (data.eventType == 'degree') {
                var val = data.dataCustom;
                var elem = $(elem).children("table").eq(0)[0];
                var lth = $(elem).find("tr").eq(1).children("td").eq(1).contents().length;
                for (var i = 0; i < lth; i++) {
            ...
            if (data.eventType == 'onChange2') {
                ...
                var val = data.dataCustom.value;
                var elem1 = $(elem).children("table").eq(0)[0];
                var tbody1 = $(elem).children("table").eq(0).children("tbody")[0];
                $(tbody1).find("select").each(function() {
                    if ($(this)[0].id == id) {
                        $(this).children("option").each(function() {
                            if ($(this).text() == val) {
                                $(this)[0].selected = 'true';
            ...
            if (data.eventType == 'about2') {
                ...
                elem.querySelector('button[title="相关流程"]').onclick();
            }
        ...
        getData_control138_5sq6Uh: function(elem) {
            if (!elem) {
                return;
            }
            var data = [];
        ...
        doAction_uiControl117_4CC1vU: function(data, elem) {
        ...
        getTemplate_uiControl117_4CC1vU: function() {
        ...
        getData_control353_VyTWpt: function(elem) {
            ...
            var title = [];
            var content = [];
            if (!elem.querySelector('#requestlogappednDiv')) {
                data.leftData = leftData;
                data.rightData = rightData;
                return data;
            }
            var divData = elem.querySelector('#requestlogappednDiv');
            var tableData = $(divData).children('table');
                    ...
                    if (i == 0) {
                        ...
                        [].forEach.call(tds, function(item, index) {
                            if (item.querySelector('table')) {
                                var suggest = item.querySelector('iframe').contentDocument.querySelector('table.ViewForm').textContent.replace(/\s/g, "").trim();
                                if (suggest == "") {
                                    trContent.push('无');
                                } else {
        ...
        doAction_uiControl355_nsECzm: function(data, elem) {
            ...
            if (eventType == 'showReceiver') {
                ...
                var docddd = elem.ownerDocument.querySelector('form[name="frmmain"]');
                var buttomTab = $(docddd).children('table')[2];
                buttomTab.querySelectorAll('td')[0].click();
            }
        },
