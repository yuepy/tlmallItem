  ...
  ysp.runtime.Model.extendLoadingModel({
    getData_control108_46cZtf: function (elem) {
                  ...
                  $(this).children("table").find("a").each(function (i) {
                    if ($(this).attr('onclick') !== undefined && ($(this).attr('onclick').indexOf('openAccessory') !== -1 || $(this).attr('onclick').indexOf('addDocReadTag') !== -1)) {
                      if ($(this).text().indexOf('txt') !== -1) {
                        ...
                        arr3.push({ name: $(this).text().trim(),
                          stl: 'pdf', no: $(this).attr('onClick').match(/\d+/g)[1] });
                      } else if ($(this).text().indexOf('xls') !== -1) {
                        arr3.push({ name: $(this).text().trim(), stl: 'xls', no: $(this).attr('onClick').match(/\d+/g)[1] });
                      } else if ($(this).text().indexOf('png') !== -1) {
                        arr3.push({ name: $(this).text().trim(), stl: 'png', no: $(this).attr('onClick').match(/\d+/g)[1] });
      ...
      if (data.eventType == 'inputBlur') {
        ...
        }, 1000);var number = data.dataCustom.number;var index = data.dataCustom.index;var row = data.dataCustom.row;
        //文件类型
        ...
        text = text.replace(/\(.*/, '');var _url = "";console.log(index, row);if (file.getAttribute("onclick").indexOf("top.location") !== -1) {
          var str = $(file)[0].getAttribute("onclick").match(/\/.*[0-9]/g);
          _url = 'http://192.168.200.63' + str;
        } else {
