...
(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control110_XrebWI: function (elem) {
      if (!elem) {
        return;
      }var data = { header: { title: [], number: [] }, base_lc_info: { title: [], content: [], degree: [], miji: { content: [], id: [] } },
        base_info: { content: [] }, related_document: [], related_process: [], file: [], state: [] };if ($(elem).find('#field27376span').length > 0 || $(elem).find('#field27381span').length > 0) {
              ...
              var tdl = $(this).find("td").length - 2;$(this).children("td").each(function (i) {
                if ($(this).find("input").length !== 3 && $(this)[0].className == 'zdn' && i == tdl) {
                  data.base_lc_info.content.push($(this).text());
                    ...
                    var arrn = [];
                    arrn.push($(this).text().trim());data.base_lc_info.degree.push(arrn);
      ...
      if (data.eventType == 'deleteFile') {
        var idx = data.dataCustom;var elem1 = $(elem).children("table").eq(1)[0];
        $(elem1).find('input').each(function () {
    ...
    getData_control325_GT6RW4: function (elem) {
      if (!elem) {
        return;
    ...
    doAction_uiControl325_xwoqDC: function (data, elem) {// {this.props.customData? <div ref="load" className="ysp_roatLoading"><span className="roatImg"></span></div>:""}
    },
    getTemplate_uiControl325_xwoqDC: function () {
