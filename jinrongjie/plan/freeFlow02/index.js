...
(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control107_5Bad4R: function (elem) {
      var data = { header: { title1: [], title2: [], number: [] }, base_lc_info: { title: [], content: [], degree: [], miji: { content: [], id: [] } }, base_info: { content: [] }, related_document: [], related_process: [], file: [] }; //附件-----------
      ...
      if ($(elem).children(".bt").length > 0) {
        var _title = $(elem).children(".bt").text().trim().replace('公文审批单', '');var _indexOf = $(elem).children(".bt").text().trim().indexOf('公文审批单');_indexOf != -1 ? data.header.title2.push('公文审批单') : null;data.header.title1.push(_title);
    ...
    doAction_uiControl92_sGLgMQ: function (data, elem) {
      if (data.eventType == 'inputBlur') {
        ...
        //   if ($(this)[0].getAttribute('onClick').match(/field\d+/) == id) {//$(this).click();
        //   }
        // });
        var id = data.dataCustom;var _button = elem.querySelector("#" + id + "").parentElement.firstElementChild;_button.nodeName == 'BUTTON' && _button.onclick();
      }if (data.eventType == 'about1') {// var _button = elem.querySelector("#signdocids").nextElementSibling;
        // _button.nodeName == 'BUTTON' && _button.onclick(); 
        // elem.ownerDocument.defaultView.eval(elem.querySelectorAll('.Browser')[2].onclick());
        // elem.querySelectorAll('.Browser')[2].onclick(); 
        // var tbody2 = $(elem).children("table").eq(1).children("tbody")[0];
        ...
        //   }
        // });
      }if (data.eventType == 'about2') {// var tbody2 = $(elem).children("table").eq(1).children("tbody")[0];
        // $(elem).find("button").each(function () {
    ...
    getTemplate_uiControl86_Z7MEmg: function () {
