...
(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    doAction_uiControl10: function doAction_uiControl10(data, elem) {
      ...
      if (eventType === 'change') {
        var el = elem.querySelector('#' + data.id);
        if (el && el.className === 'Wdate') {
          var value = data.value.replace(/\-/g, '');
          el.value = value;
          if (el.parentNode && el.parentNode.nextElementSibling) {
            el.parentNode.nextElementSibling.value = value;
          }
        }
      }
    },
